...
curl -s http://localhost:8080/api/news/health
echo -e "\n"

# Test 2: Get All News
echo "2️⃣ Testing Get All News..."
curl -s http://localhost:8080/api/news/all | jq '. | length' 2>/dev/null || echo "No articles found or jq not installed"
echo -e "\n"

# Test 3: Contextual Query - Technology News
echo "3️⃣ Testing Contextual Query - Technology News..."
curl -s -X POST http://localhost:8080/api/news/query \
...
curl -s http://localhost:8080/api/news/category/Technology | jq '. | length' 2>/dev/null || echo "Category retrieval failed"
echo -e "\n"

# Test 6: Source Retrieval
echo "6️⃣ Testing Source Retrieval - Reuters..."
curl -s http://localhost:8080/api/news/source/Reuters | jq '. | length' 2>/dev/null || echo "Source retrieval failed"
echo -e "\n"

# Test 7: Score-based Retrieval
echo "7️⃣ Testing Score-based Retrieval (threshold: 0.8)..."
curl -s "http://localhost:8080/api/news/score?threshold=0.8" | jq '. | length' 2>/dev/null || echo "Score retrieval failed"
echo -e "\n"

# Test 8: Text Search
echo "8️⃣ Testing Text Search - 'Apple'..."
curl -s "http://localhost:8080/api/news/search?q=Apple" | jq '. | length' 2>/dev/null || echo "Text search failed"
echo -e "\n"

# Test 9: Nearby News
echo "9️⃣ Testing Nearby News (Palo Alto area)..."
curl -s "http://localhost:8080/api/news/nearby?latitude=37.4419&longitude=-122.1430&radius=10" | jq '. | length' 2>/dev/null || echo "Nearby news failed"
echo -e "\n"
